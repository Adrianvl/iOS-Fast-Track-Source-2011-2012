...
- (IBAction)doStartButton:(id)sender {
    ...
    void(^block1A)(void) = ^(void) {
        for (size_t n=0; n<100; n++) {
            ...
            if (self.userCancelled == YES) break;
        }        
    };
    ...
    //Final block - this depends on ALL other blocks either completing or being cancelled
    NSBlockOperation *blockOperationFinal = [NSBlockOperation blockOperationWithBlock:blockFinal];
    [blockOperationFinal addDependency:blockOperation1A];
    [blockOperationFinal addDependency:blockOperation1B];
    [blockOperationFinal addDependency:blockOperation2A];
