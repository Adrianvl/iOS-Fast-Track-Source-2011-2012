...
- (IBAction)doButton:(id)sender 
{
    ...
    NSArray *arrayOfBlocks = [[NSArray alloc] initWithObjects:calcFactorialAsString, calcSqrtAsString, calcTwoPowerAsString, nil];
    ...
    //Tidy up - this will release the contained block objects before they go out of scope
    [arrayOfBlocks release];
    
    // **NOTES - the following line would have resulted in a crash
    // NSArray *blocks = [NSArray arrayWithObjects:calcFactorialAsString, calcSqrtAsString, calcTwoPowerAsString, nil];  
    // This would implicitly retain the blocks (but not copy) - when the functions exits, the blocks on the stack will be out of scope
    // The autolrelease pool will ultimately deallocate the array, which in turn will try to release the blocks - and a run time crash will occur
