...
- (IBAction)doSplat:(id)sender {
    ...
    ModalViewControllerCallBackBlock blk = ^(NSString* msg, BOOL didChange) {
        ...
        //A block called when the animation is complete
        void(^modalDismissAnimationDone)(void) = ^ {
            NSLog(@"Animation Complete");
        };
//      [self dismissModalViewControllerAnimated:YES];
        [self dismissViewControllerAnimated:YES completion:modalDismissAnimationDone];
    };
    
    //Animation completion block
    void(^modalAnimationDone)(void) = ^ {
        NSLog(@"Animation Complete");
    };
    ...
    [self presentViewController:modal animated:YES completion:modalAnimationDone];
